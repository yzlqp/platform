...
int ComQUIHelperData::strHexToDecimal(const QString &strHex)
{
    bool ok;
    return strHex.toInt(&ok, 16);
}

int ComQUIHelperData::strDecimalToDecimal(const QString &strDecimal)
{
    bool ok;
    return strDecimal.toInt(&ok, 10);
}

int ComQUIHelperData::strBinToDecimal(const QString &strBin)
{
    bool ok;
    return strBin.toInt(&ok, 2);
}

QString ComQUIHelperData::strHexToStrBin(const QString &strHex)
{
...
QString ComQUIHelperData::decimalToStrBin1(int decimal)
{
    QString bin = QString::number(decimal, 2);
    uchar len = bin.length();
    if (len <= 8) {
...
QString ComQUIHelperData::decimalToStrBin2(int decimal)
{
    QString bin = QString::number(decimal, 2);
    uchar len = bin.length();
    if (len <= 16) {
...
QString ComQUIHelperData::decimalToStrHex(int decimal)
{
...
QByteArray ComQUIHelperData::intToByte(int data)
{
    QByteArray result;
    result.resize(4);
...
QByteArray ComQUIHelperData::intToByteRec(int data)
{
    QByteArray result;
    result.resize(4);
...
int ComQUIHelperData::byteToInt(const QByteArray &data)
{
...
int ComQUIHelperData::byteToIntRec(const QByteArray &data)
{
...
quint32 ComQUIHelperData::byteToUInt(const QByteArray &data)
{
...
quint32 ComQUIHelperData::byteToUIntRec(const QByteArray &data)
{
...
QByteArray ComQUIHelperData::ushortToByte(ushort data)
{
    QByteArray result;
    result.resize(2);
...
QByteArray ComQUIHelperData::ushortToByteRec(ushort data)
{
    QByteArray result;
    result.resize(2);
    result[0] = (uchar)(0x000000ff & data);
...
int ComQUIHelperData::byteToUShort(const QByteArray &data)
{
...
int ComQUIHelperData::byteToUShortRec(const QByteArray &data)
{
    int i = data.at(0) & 0x000000FF;
    i |= ((data.at(1) << 8) & 0x0000FF00);
    if (i >= 32768) {
...
QString ComQUIHelperData::getValue(quint8 value)
{
...
QString ComQUIHelperData::getXorEncryptDecrypt(const QString &value, char key)
{
    //矫正范围外的数据
    if (key < 0 || key >= 127) {
        key = 127;
...
uchar ComQUIHelperData::getOrCode(const QByteArray &data)
{
    int len = data.length();
    uchar result = 0;
    for (int i = 0; i < len; ++i) {
...
uchar ComQUIHelperData::getCheckCode(const QByteArray &data)
{
    int len = data.length();
    uchar temp = 0;
...
//正序CRC计算
quint16 ComQUIHelperData::getCrc16(quint8 *data, int len, quint16 init, const quint16 *table)
{
    quint16 crc_16 = init;
    quint8 temp;
    while (len-- > 0) {
...
//逆序CRC计算
quint16 ComQUIHelperData::getCrc16Rec(quint8 *data, int len, quint16 init, const quint16 *table)
{
    quint16 crc_16 = init;
    quint8 temp;
    while (len-- > 0) {
...
//Modbus CRC16校验
quint16 ComQUIHelperData::getModbus16(quint8 *data, int len)
{
...
//CRC16校验
QByteArray ComQUIHelperData::getCrcCode(const QByteArray &data)
{
    quint16 result = getModbus16((quint8 *)data.data(), data.length());
    return ComQUIHelperData::ushortToByteRec(result);
}

static QMap<char, QString> listChar;
void ComQUIHelperData::initAscii()
{
    //0x20为空格,空格以下都是不可见字符
    if (listChar.count() == 0) {
...
QString ComQUIHelperData::byteArrayToAsciiStr(const QByteArray &data)
{
    //先初始化字符表
    initAscii();
...
QByteArray ComQUIHelperData::asciiStrToByteArray(const QString &data)
{
    //先初始化字符表
    initAscii();
...
char ComQUIHelperData::hexStrToChar(char data)
{
    if ((data >= '0') && (data <= '9')) {
        return data - 0x30;
...
QByteArray ComQUIHelperData::hexStrToByteArray(const QString &data)
{
    QByteArray senddata;
...
QString ComQUIHelperData::byteArrayToHexStr(const QByteArray &data)
{
    QString temp = "";
