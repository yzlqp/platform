...
int ComQUIHelper::getScreenIndex()
{
    //需要对多个屏幕进行处理
    int screenIndex = 0;
...
QRect ComQUIHelper::getScreenRect(bool available)
{
    QRect rect;
    int screenIndex = ComQUIHelper::getScreenIndex();
    if (available) {
#if (QT_VERSION >= QT_VERSION_CHECK(5,0,0))
...
int ComQUIHelper::deskWidth()
{
    return getScreenRect().width();
}

int ComQUIHelper::deskHeight()
{
    return getScreenRect().height();
}

QSize ComQUIHelper::deskSize()
{
    return getScreenRect().size();
}

QWidget *ComQUIHelper::centerBaseForm = 0;
void ComQUIHelper::setFormInCenter(QWidget *form)
{
...
void ComQUIHelper::showForm(QWidget *form)
{
...
QList<QColor> ComQUIHelper::colors = QList<QColor>();
QList<QColor> ComQUIHelper::getColorList()
{
    //备用颜色集合 可以自行添加
    if (colors.count() == 0) {
...
QStringList ComQUIHelper::getColorNames()
{
...
QColor ComQUIHelper::getRandColor()
{
    QList<QColor> colors = getColorList();
...
float ComQUIHelper::getRandFloat(float min, float max)
{
...
double ComQUIHelper::getRandValue(int min, int max, bool contansMin, bool contansMax)
{
    int value;
...
QStringList ComQUIHelper::getRandPoint(int count, float mainLng, float mainLat, float dotLng, float dotLat)
{
    //随机生成点坐标
    QStringList points;
...
void ComQUIHelper::newDir(const QString &dirName)
{
    ...
    if (!strDir.startsWith("/") && !strDir.contains(":/")) {
        strDir = QString("%1/%2").arg(ComQUIHelper::appPath()).arg(strDir);
    }
...
void ComQUIHelper::sleep(int msec)
{
    if (msec <= 0) {
        return;
    }
...
QFont ComQUIHelper::addFont(const QString &fontFile, const QString &fontName)
{
    //判断图形字体是否存在,不存在则加入
...
void ComQUIHelper::setFont(int fontSize)
{
#ifdef rk3399
    return;
#endif
...
void ComQUIHelper::setCode(bool utf8)
{
#if (QT_VERSION >= QT_VERSION_CHECK(5,0,0))
...
void ComQUIHelper::setTranslator(const QString &qmFile)
{
    //过滤下不存在的就不用设置了
...
void ComQUIHelper::initAll(bool utf8, bool style, int fontSize)
{
    //初始化随机数种子
    ComQUIHelper::initRand();
    //设置编码
    ComQUIHelper::setCode(utf8);
    //设置样式风格
    if (style) {
        ComQUIHelper::setStyle();
    }
    //设置字体
    ComQUIHelper::setFont(fontSize);
    //设置翻译文件支持多个
    ComQUIHelper::setTranslator(":/qm/widgets.qm");
    ComQUIHelper::setTranslator(":/qm/qt_zh_CN.qm");
    ComQUIHelper::setTranslator(":/qm/designer_zh_CN.qm");
}

void ComQUIHelper::setFramelessForm(QWidget *widgetMain, bool tool, bool top, bool menu)
{
...
int ComQUIHelper::showMessageBox(const QString &info, int type, int closeSec, bool exec)
{
    int result = 0;
    if (type == 0) {
...
void ComQUIHelper::showMessageBoxInfo(const QString &info, int closeSec, bool exec)
{
...
void ComQUIHelper::showMessageBoxError(const QString &info, int closeSec, bool exec)
{
...
int ComQUIHelper::showMessageBoxQuestion(const QString &info)
{
...
QString ComQUIHelper::getXorEncryptDecrypt(const QString &value, char key)
{
    //矫正范围外的数据
    if (key < 0 || key >= 127) {
        key = 127;
...
uchar ComQUIHelper::getOrCode(const QByteArray &data)
{
    int len = data.length();
    uchar result = 0;
    for (int i = 0; i < len; i++) {
...
uchar ComQUIHelper::getCheckCode(const QByteArray &data)
{
    int len = data.length();
    uchar temp = 0;
...
void ComQUIHelper::initTableView(QTableView *tableView, int rowHeight, bool headVisible, bool edit, bool stretchLast)
{
    //设置弱属性用于应用qss特殊样式
...
void ComQUIHelper::openFile(const QString &fileName, const QString &msg)
{
#ifdef __arm__
    return;
#endif
    ...
    if (ComQUIHelper::showMessageBoxQuestion(msg + "成功!确定现在就打开吗?") == QMessageBox::Yes) {
        QString url = QString("file:///%1").arg(fileName);
        QDesktopServices::openUrl(QUrl(url, QUrl::TolerantMode));
    }
}

bool ComQUIHelper::checkIniFile(const QString &iniFile)
{
