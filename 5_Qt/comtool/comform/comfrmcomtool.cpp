...
#include "ui_comfrmcomtool.h"
#include "comquihelper.h"
#include "comquihelperdata.h"
...
frmComTool::frmComTool(QWidget *parent) : QWidget(parent), ui(new Ui::frmComTool)
{
    ...
    this->initForm();
    this->initConfig();
    ComQUIHelper::setFormInCenter(this);
}
...
void frmComTool::initForm()
{
    ...
    ui->cboxSendInterval->addItems(ComAppData::Intervals);
    ui->cboxData->addItems(ComAppData::Datas);
...
void frmComTool::initConfig()
{
    ...
    ui->cboxPortName->setCurrentIndex(ui->cboxPortName->findText(ComAppConfig::PortName));
    ...
    ui->cboxBaudRate->setCurrentIndex(ui->cboxBaudRate->findText(QString::number(ComAppConfig::BaudRate)));
    ...
    ui->cboxDataBit->setCurrentIndex(ui->cboxDataBit->findText(QString::number(ComAppConfig::DataBit)));
    ...
    ui->cboxParity->setCurrentIndex(ui->cboxParity->findText(ComAppConfig::Parity));
    ...
    ui->cboxStopBit->setCurrentIndex(ui->cboxStopBit->findText(QString::number(ComAppConfig::StopBit)));
    connect(ui->cboxStopBit, SIGNAL(currentIndexChanged(int)), this, SLOT(saveConfig()));

    ui->ckHexSend->setChecked(ComAppConfig::HexSend);
    connect(ui->ckHexSend, SIGNAL(stateChanged(int)), this, SLOT(saveConfig()));

    ui->ckHexReceive->setChecked(ComAppConfig::HexReceive);
    connect(ui->ckHexReceive, SIGNAL(stateChanged(int)), this, SLOT(saveConfig()));

    ui->ckDebug->setChecked(ComAppConfig::Debug);
    connect(ui->ckDebug, SIGNAL(stateChanged(int)), this, SLOT(saveConfig()));

    ui->ckAutoClear->setChecked(ComAppConfig::AutoClear);
    connect(ui->ckAutoClear, SIGNAL(stateChanged(int)), this, SLOT(saveConfig()));

    ui->ckAutoSend->setChecked(ComAppConfig::AutoSend);
    connect(ui->ckAutoSend, SIGNAL(stateChanged(int)), this, SLOT(saveConfig()));

    ui->ckAutoSave->setChecked(ComAppConfig::AutoSave);
    ...
    ui->cboxSendInterval->setCurrentIndex(ui->cboxSendInterval->findText(QString::number(ComAppConfig::SendInterval)));
    connect(ui->cboxSendInterval, SIGNAL(currentIndexChanged(int)), this, SLOT(saveConfig()));
    ui->cboxSaveInterval->setCurrentIndex(ui->cboxSaveInterval->findText(QString::number(ComAppConfig::SaveInterval)));
    connect(ui->cboxSaveInterval, SIGNAL(currentIndexChanged(int)), this, SLOT(saveConfig()));

    timerSend->setInterval(ComAppConfig::SendInterval);
    timerSave->setInterval(ComAppConfig::SaveInterval);

    if (ComAppConfig::AutoSend) {
        timerSend->start();
    }

    if (ComAppConfig::AutoSave) {
        timerSave->start();
    }

    //串口转网络部分
    ui->cboxMode->setCurrentIndex(ui->cboxMode->findText(ComAppConfig::Mode));
    connect(ui->cboxMode, SIGNAL(currentIndexChanged(int)), this, SLOT(saveConfig()));

    ui->txtServerIP->setText(ComAppConfig::ServerIP);
    connect(ui->txtServerIP, SIGNAL(textChanged(QString)), this, SLOT(saveConfig()));

    ui->txtServerPort->setText(QString::number(ComAppConfig::ServerPort));
    connect(ui->txtServerPort, SIGNAL(textChanged(QString)), this, SLOT(saveConfig()));

    ui->txtListenPort->setText(QString::number(ComAppConfig::ListenPort));
    ...
    ui->cboxSleepTime->setCurrentIndex(ui->cboxSleepTime->findText(QString::number(ComAppConfig::SleepTime)));
    connect(ui->cboxSleepTime, SIGNAL(currentIndexChanged(int)), this, SLOT(saveConfig()));

    ui->ckAutoConnect->setChecked(ComAppConfig::AutoConnect);
    connect(ui->ckAutoConnect, SIGNAL(stateChanged(int)), this, SLOT(saveConfig()));
}

void frmComTool::saveConfig()
{
    ComAppConfig::PortName = ui->cboxPortName->currentText();
    ComAppConfig::BaudRate = ui->cboxBaudRate->currentText().toInt();
    ComAppConfig::DataBit = ui->cboxDataBit->currentText().toInt();
    ComAppConfig::Parity = ui->cboxParity->currentText();
    ComAppConfig::StopBit = ui->cboxStopBit->currentText().toDouble();

    ComAppConfig::HexSend = ui->ckHexSend->isChecked();
    ComAppConfig::HexReceive = ui->ckHexReceive->isChecked();
    ComAppConfig::Debug = ui->ckDebug->isChecked();
    ComAppConfig::AutoClear = ui->ckAutoClear->isChecked();

    ComAppConfig::AutoSend = ui->ckAutoSend->isChecked();
    ComAppConfig::AutoSave = ui->ckAutoSave->isChecked();

    int sendInterval = ui->cboxSendInterval->currentText().toInt();
    if (sendInterval != ComAppConfig::SendInterval) {
        ComAppConfig::SendInterval = sendInterval;
        timerSend->setInterval(ComAppConfig::SendInterval);
    }

    int saveInterval = ui->cboxSaveInterval->currentText().toInt();
    if (saveInterval != ComAppConfig::SaveInterval) {
        ComAppConfig::SaveInterval = saveInterval;
        timerSave->setInterval(ComAppConfig::SaveInterval);
    }

    ComAppConfig::Mode = ui->cboxMode->currentText();
    ComAppConfig::ServerIP = ui->txtServerIP->text().trimmed();
    ComAppConfig::ServerPort = ui->txtServerPort->text().toInt();
    ComAppConfig::ListenPort = ui->txtListenPort->text().toInt();
    ComAppConfig::SleepTime = ui->cboxSleepTime->currentText().toInt();
    ComAppConfig::AutoConnect = ui->ckAutoConnect->isChecked();

    ComAppConfig::writeConfig();
}
...
void frmComTool::readData()
{
    ...
    if (isShow) {
        ...
        if (ui->ckHexReceive->isChecked()) {
            buffer = ComQUIHelperData::byteArrayToHexStr(data);
        } else {
        ...
        if (ui->ckDebug->isChecked()) {
            int count = ComAppData::Keys.count();
            for (int i = 0; i < count; i++) {
                if (buffer.startsWith(ComAppData::Keys.at(i))) {
                    sendData(ComAppData::Values.at(i));
                    break;
                }
            }
        }
...
void frmComTool::sendData(QString data)
{
    ...
    if (ui->ckHexSend->isChecked()) {
        buffer = ComQUIHelperData::hexStrToByteArray(data);
    } else {
        buffer = ComQUIHelperData::asciiStrToByteArray(data);
    }
...
void frmComTool::saveData()
{
    ...
    QString fileName = QString("%1/%2.txt").arg(ComQUIHelper::appPath()).arg(name);
...
{
    QString fileName = QString("%1/%2").arg(ComQUIHelper::appPath()).arg("send.txt");
    QFile file(fileName);
    if (!file.exists()) {
        return;
    }
...
void frmComTool::on_btnStart_clicked()
{
    if (ui->btnStart->text() == "启动") {
        if (ComAppConfig::ServerIP == "" || ComAppConfig::ServerPort == 0) {
            append(6, "IP地址和远程端口不能为空");
            return;
        }

        socket->connectToHost(ComAppConfig::ServerIP, ComAppConfig::ServerPort);
...
{
    if (!tcpOk && ComAppConfig::AutoConnect && ui->btnStart->text() == "启动") {
        if (ComAppConfig::ServerIP != "" && ComAppConfig::ServerPort != 0) {
            socket->connectToHost(ComAppConfig::ServerIP, ComAppConfig::ServerPort);
            if (socket->waitForConnected(100)) {
                ui->btnStart->setText("停止");
...
void frmComTool::readDataNet()
{
    if (socket->bytesAvailable() > 0) {
        ComQUIHelper::sleep(ComAppConfig::SleepTime);
        ...
        if (ui->ckHexReceive->isChecked()) {
            ...
            buffer = ComQUIHelperData::byteArrayToAsciiStr(data);
        }
